...
function safeTree(value: unknown, depth = 0): boolean {
	...
	if (
		depth >= 4 ||
		(Array.isArray(value) && value.length > MAX_TASKS) ||
		(!Array.isArray(value) && !isPlainObject(value))
	)
		return false;
	return Array.isArray(value)
		? value.every((item) => safeTree(item, depth + 1))
		: Reflect.ownKeys(value).length <= MAX_FIELDS &&
	...
	accept(
		...
		try {
			...
			if (
				...
				details.action.length > 64 ||
				!isPlainObject(details.params) ||
				!safeTree(details.params) ||
				!Array.isArray(details.tasks) ||
				details.tasks.length > MAX_TASKS ||
				!Number.isSafeInteger(details.nextId) ||
				(details.nextId as number) < 1 ||
				(details.nextId as number) > Number.MAX_SAFE_INTEGER ||
				(details.error !== undefined && !safeTree(details.error))
			)
				return null;
			let active = 0;
			let completed = 0;
			...
			for (const rawTask of details.tasks) {
				if (!isPlainObject(rawTask) || !ownData(rawTask, TASK_KEYS)) {
					return null;
				}
				...
				if (id < 1 || taskIds.has(id)) return null;
			...
			let state: PresenceStateInputV2["state"];
			if (active > 0) state = "running";
			else if (visible === 0) state = "idle";
			else if (completed === visible) state = "success";
			else state = "waiting";
