...
export function compositeState(events: readonly PresenceUpdate[], active: boolean, locallyBlocked=false): HerdrState { if(locallyBlocked || events.some(e=>e.state==="error" || e.attention==="error"))return "blocked"; if(events.some(isInteractionWaiting))return "blocked"; if(active || events.some(e=>e.state==="running" || e.state==="waiting"))return "working"; return "idle"; }
/** Native blocked labels are never forwarded: only an allowlisted category selects wording. */
export function safeMessage(state:HerdrState,max:number,blockedCategory?:BlockedCategory,events:readonly PresenceUpdate[]=[]):string { const category=blockedPresentationCategory(events,blockedCategory);return boundedPresenceText(state==="blocked" ? (category==="ask-user" ? "Pi needs your input" : "Pi needs attention") : state==="working" ? "Pi is working" : state==="idle" ? "Pi is idle" : "Pi state unknown",{maxBytes:128,maxCodePoints:max}); }
...
export function metadata(events:readonly PresenceUpdate[], state:HerdrState,max:number,summary:SubagentSummary|null=null,blockedCategory?:BlockedCategory){ const count=(k:keyof PresenceUpdate["counts"])=>events.reduce((n,e)=>n+(typeof e.counts[k]==="number"?e.counts[k] as number:0),0);const tokens=events.reduce((n,e)=>n+(e.usage?.tokens??0),0);const cost=events.reduce((n,e)=>n+(e.usage?.cost??0),0);const context=Math.max(0,...events.map(e=>e.usage?.contextPercent??0));const progress=events.find(e=>e.source.id==="pi-todo"&&e.progress)??events.find(e=>e.progress&&(e.state==="running"||e.state==="waiting")); const category=blockedPresentationCategory(events,blockedCategory);const labels:Record<string,string>={idle:"Idle",working:"Working",blocked:category==="ask-user"?"Input needed":"Needs attention",unknown:"Unknown"}; return {title:boundedPresenceText(`Pi · ${labels[state]}`,{maxBytes:128,maxCodePoints:max}),displayAgent:"Pi",labels,tokens:{active:String(count("active")),completed:String(count("completed")),failed:String(count("failed")),queued:String(count("queued")),cancelled:String(count("cancelled")),total:String(count("total")),progress:progress?.progress?`${Math.round(progress.progress.value*100)}%`:null,tokens:tokens?String(Math.round(tokens)):null,cost:cost?cost.toFixed(2):null,context:context?`${Math.round(context)}%`:null,...summaryTokens(summary)}}; }
