...
export class PresenceClient {
  ...
  private async performTeardown(timeoutMs:number):Promise<void>{
   ...
   try {
    ...
    if(!expired&&!this.closed&&!this.companion)await Promise.race([this.clearLegacyMetadataOnTeardown().catch(()=>{}),expires]);
    if(!expired&&!this.closed&&!this.companion)await Promise.race([this.clearAgentAuthority().catch(()=>{}),expires]);
