...
import { PI_PRESENCE_SUMMARY_EVENT } from "./subagent-summary.js";
...
export function registerPresenceHooks(pi: ExtensionAPI, runtime: PresenceRuntime): void {
 pi.events.on(PI_PRESENCE_UPDATE_EVENT,p=>runtime.handlePresenceUpdate(p)); pi.events.on(PI_PRESENCE_REMOVE_EVENT,p=>runtime.handlePresenceRemove(p)); pi.events.on(PI_PRESENCE_SUMMARY_EVENT,p=>runtime.handleSubagentSummary(p)); pi.events.on(PI_PRESENCE_READY_EVENT,p=>runtime.handleReady(p)); pi.events.on("herdr:blocked",p=>runtime.handleBlocked(p));
