...
export function resolvePresenceConfig(env: NodeJS.ProcessEnv = process.env): PresenceConfig { return { enabled: bool(env, SETTINGS.enabled), timeoutMs: integer(env, SETTINGS.timeoutMs), maxQueue: integer(env, SETTINGS.maxQueue), notifications: bool(env, SETTINGS.notifications), notificationPolicy: enumeration(env, SETTINGS.notificationPolicy), metadata: bool(env, SETTINGS.metadata), finalClearMs: integer(env, SETTINGS.finalClearMs), maxLabelChars: integer(env, SETTINGS.maxLabelChars) }; }
