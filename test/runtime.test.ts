...
test("extension registers V2 listeners and lifecycle hooks",()=>{const events:string[]=[];const hooks:string[]=[];const old={...process.env};try{Object.assign(process.env,{HERDR_ENV:"1",HERDR_SOCKET_PATH:"/tmp/x",HERDR_PANE_ID:"p",HERDR_WORKSPACE_ID:"workspace"});extension({on:(n:string)=>hooks.push(n),events:{on:(n:string)=>events.push(n),emit(){}}} as never);expect(events).toEqual(expect.arrayContaining([EVENT_NAMES.state,EVENT_NAMES.terminal,EVENT_NAMES.withdraw,EVENT_NAMES.consumerReady]));expect(hooks).toEqual(["agent_settled","session_start","agent_start","turn_start","message_end","agent_end","tool_result","ui_prompt_start","ui_prompt_end","session_shutdown"]);}finally{process.env=old;}});
