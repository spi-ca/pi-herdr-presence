...
No task or subagent argument, prompt, output, path, session name or path, arbitrary ID, or raw error is read or projected. In both modes, the title is exactly `Pi · ${summary}`, derived only from the bounded safe `summary` token; `display_agent` is fixed to `Pi` and `state_labels` are fixed. Companion applies this presentation and the exact ten-token map under `herdr:pi-presence` to managed `herdr:pi`, without lifecycle or session authority.
