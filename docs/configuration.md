...
The current compatibility target is Herdr application `v0.9.1` with protocol `22`. This is distinct from `HERDR_INTEGRATION_VERSION=9` in Herdr's managed `herdr-agent-state.ts` asset: `9` identifies that managed integration asset and does not identify the Herdr application or socket protocol version.
