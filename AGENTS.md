...
Core invariants: only `HERDR_ENV=1`, `HERDR_SOCKET_PATH`, explicit opaque `HERDR_WORKSPACE_ID`, and `HERDR_PANE_ID`; no process/CLI/shell execution or subscriptions (except the bounded workspace-summary lease heartbeat); request-per-connection transport; strict target/socket validation; output-only best-effort failures. Exact managed `herdr-agent-state.ts` presence selects companion coexistence: under `herdr:pi-presence`, applied to `herdr:pi`, it owns the bounded fixed presentation and exact ten-token metadata projection and may emit policy-gated static notifications. It never emits or clears session/state authority, legacy cleanup, authority clear, focus/control, or arbitrary text.
